...
class Unigram(LM):
    ...
    def __init__(self: UnigramType,
        ...
        # Iterate through all scripts and build the vocabulary
        for line in data:
            for w in list(line) + [END_TOKEN]:
                ...
                if w == END_TOKEN:
                    count[w_idk] += min(1, MAX_COUNT / (count[w_idk] + EOS_DISCOUNT))
                else:
                    count[w_idk] += min(1, MAX_COUNT / (count[w_idk] + 1))

                total += 1

        # Compute log probabilities for each word in the vocabulary
        v = len(self.vocab)

        self.logprob: np.ndarray = np.zeros(v, dtype=float)
        for w_idx in range(v):
            self.logprob[w_idx] = np.log((count[w_idx] + alpha) / (total + alpha * v))
